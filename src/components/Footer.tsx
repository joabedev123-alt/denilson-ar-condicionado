...
export default function Footer() {
  return (
    <footer className="relative border-t border-blue-500/10">
      <div className="absolute inset-0 bg-gradient-to-b from-[#020C1B] to-[#010812]" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pt-12 sm:pt-16 pb-8">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-600 to-sky-400 flex items-center justify-center">
            ...
            <div className="flex gap-3">
              <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 rounded-xl glass border border-blue-500/20 flex items-center justify-center text-slate-400 hover:text-green-400 hover:border-green-400/30 transition-all duration-200 cursor-pointer"
                aria-label="WhatsApp">
                <i className="bi bi-whatsapp text-base" />
              </a>
              <a href={`tel:${PHONE.replace(/\D/g, "")}`}
                className="w-9 h-9 rounded-xl glass border border-blue-500/20 flex items-center justify-center text-slate-400 hover:text-blue-400 hover:border-blue-400/30 transition-all duration-200 cursor-pointer"
                aria-label="Telefone">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {links.map((link) => (
                <li key={link.label}>
                  <a href={link.href}
                    className="text-slate-500 hover:text-blue-400 text-sm transition-colors duration-200 cursor-pointer flex items-center gap-1.5 group">
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <a href={`mailto:${EMAIL}`}
                className="flex items-center gap-2.5 text-slate-500 hover:text-blue-400 text-sm transition-colors duration-200 cursor-pointer">
                <i className="bi bi-envelope text-blue-500 text-base" />
                {EMAIL}
              </a>
              <div className="flex items-start gap-2.5 text-slate-500 text-sm">
                <i className="bi bi-geo-alt text-blue-500 text-base mt-0.5" />
                <span>Grande Sao Paulo e ABC Paulista</span>
              </div>
              <div className="flex items-center gap-2.5 text-slate-500 text-sm">
                <i className="bi bi-clock text-blue-500 text-base" />
        ...
        {/* Bottom bar */}
        <div className="border-t border-blue-500/10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
